...
from PunyaChael import SPAM_LOG_CHANNEL, SUDOERS, PunyaChael
from PunyaChael.modules.info import get_info
from PunyaChael.utils.db import (add_chat, add_user, blacklist_chat,
                          blacklist_user, chat_exists,
                          is_chat_blacklisted, is_user_blacklisted,
                          user_exists, whitelist_chat, whitelist_user)


@PunyaChael.on_message(
...
async def blacklist_func(_, message: Message):
    ...
    if id < 0:
        try:
            chat = await PunyaChael.get_chat(id)
        ...
        return await PunyaChael.send_message(SPAM_LOG_CHANNEL, text=msg)
    ...
    try:
        user = await PunyaChael.get_users(id)
    except Exception as e:
        return await message.reply_text(str(e))
    ...
    msg = f"**BLACKLIST EVENT**\n{await get_info(id)}"
    await PunyaChael.send_message(SPAM_LOG_CHANNEL, text=msg)
