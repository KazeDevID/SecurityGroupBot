...
from PunyaChael import SUDOERS, arq, PunyaChael
from PunyaChael.utils.db import (disable_nsfw, disable_spam, enable_nsfw,
                          enable_spam, is_nsfw_enabled,
                          is_spam_enabled)
from PunyaChael.utils.misc import admins, get_file_id
...
@PunyaChael.on_message(filters.command("nsfw_scan"), group=3)
async def nsfw_scan_command(_, message: Message):
    ...
    file = await PunyaChael.download_media(file_id)
    try:
...
@PunyaChael.on_message(filters.command("spam_scan"), group=3)
