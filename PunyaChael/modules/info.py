...
from PunyaChael import SUDOERS, PunyaChael
...
@Protection_ByKazeBot [CHAT_ID/Username|USER_ID/Username]
"""


async def get_user_info(user):
    try:
        user = await PunyaChael.get_users(user)
    except Exception:
        return
...
async def get_chat_info(chat):
    try:
        chat = await PunyaChael.get_chat(chat)
    except Exception:
        return
...
@PunyaChael.on_message(filters.command("info"), group=3)
