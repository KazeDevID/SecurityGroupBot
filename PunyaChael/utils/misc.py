...
from PunyaChael import DB_NAME, SESSION_NAME, SUDOERS, PunyaChael
...
async def backup():
    for user in SUDOERS:
        try:
            await gather(
                PunyaChael.send_document(user, DB_NAME),
                PunyaChael.send_document(user, SESSION_NAME + ".session"),
            )
        except Exception:
            pass
...
async def admins(chat_id: int):
    ...
    admins_in_chat[chat_id] = {
        ...
        "data": [
            member.user.id
            async for member in PunyaChael.iter_chat_members(
...
@PunyaChael.on_chat_member_updated()
