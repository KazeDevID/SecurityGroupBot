...
from PunyaChael import NSFW_LOG_CHANNEL, SPAM_LOG_CHANNEL, PunyaChael
from PunyaChael.core import ikb
from PunyaChael.utils.db import (get_blacklist_event, get_nsfw_count,
...
async def delete_get_info(message: Message):
    try:
        ...
        try:
            ...
            return await PunyaChael.leave_chat(message.chat.id)
...
"""
    await PunyaChael.send_message(message.chat.id, text=msg)
...
{content}
    ...
    m = await PunyaChael.send_message(
        SPAM_LOG_CHANNEL,
        report,
        reply_markup=keyb,
...
async def kick_user_notify(message: Message):
    try:
        await PunyaChael.ban_chat_member(
...
"""
    await PunyaChael.send_message(message.chat.id, msg)
